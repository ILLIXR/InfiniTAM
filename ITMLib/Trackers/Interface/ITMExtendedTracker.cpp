...
{
    //pyh Initializing ICP statistic parameters
    total_iter=0;
    total_tracker_ICP_time=std::chrono::nanoseconds(0);
...
//pyh added function
void ITMExtendedTracker::SetTrackingState(ITMTrackingState *trackingState, const ITMView *view)
{
    this->trackingState = trackingState;
    this->view = view;
    this->trackingState->trackerResult = ITMTrackingState::TRACKING_GOOD;
}
std::chrono::nanoseconds ITMExtendedTracker::GetICPLoopTime()
{
    return this->total_tracker_ICP_time;
}
unsigned ITMExtendedTracker::GetICPIterNo()
{
    return this->total_iter;
}
void ITMExtendedTracker::SetInvPose(Matrix4f& Inv_M)
{
    this->trackingState->trackerResult = ITMTrackingState::TRACKING_GOOD;
...
void ITMExtendedTracker::PrepareForEvaluation()
{
    ...
    for (int i = 1; i < viewHierarchy_Depth->GetNoLevels(); i++)
    {
        ...
        //currentLevel->depth = previousLevel->depth;
...
void ITMExtendedTracker::TrackCamera(ITMTrackingState *trackingState, const ITMView *view)
{
    ...
    for (int levelId = viewHierarchy_Depth->GetNoLevels() - 1; levelId >= 0; levelId--)
    {
        ...
        //testr(0,0) = lastKnownGoodPose.GetInvM().m[0]; testr(0,1) = lastKnownGoodPose.GetInvM().m[4]; testr(0,2) = lastKnownGoodPose.GetInvM().m[8];
        //testr(1,0) = lastKnownGoodPose.GetInvM().m[1]; testr(1,1) = lastKnownGoodPose.GetInvM().m[5]; testr(1,2) = lastKnownGoodPose.GetInvM().m[9];
        //testr(2,0) = lastKnownGoodPose.GetInvM().m[2]; testr(2,1) = lastKnownGoodPose.GetInvM().m[6]; testr(2,2) = lastKnownGoodPose.GetInvM().m[10];
        //Eigen::Quaternionf testq(testr);
        //std::cout <<"LastKnown Pose orentaion: "<< testq.normalized().x() <<" "<<testq.normalized().y()<<" "<<testq.normalized().z()<<" "<<testq.normalized().w()<<"\n";
        //
        //std::cout <<"approxInvPose translation: "<<approxInvPose.m[12] <<" "<<approxInvPose.m[13]<<" "<<approxInvPose.m[14]<<"\n";
        //Eigen::Matrix3f testr1;
        //testr1(0,0) = approxInvPose.m[0]; testr1(0,1) = approxInvPose.m[4]; testr1(0,2) = approxInvPose.m[8];
        //testr1(1,0) = approxInvPose.m[1]; testr1(1,1) = approxInvPose.m[5]; testr1(1,2) = approxInvPose.m[9];
